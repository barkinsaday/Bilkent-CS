...
for M = [21, 31, 51]
    ...
    imshow(Y);
    m = M;
    caption = sprintf('1-D (Horizontal) M-point averaging filter applied, M= %d', m);
    title(caption, 'FontSize', 14);
    imageno = imageno + 1;
end
...
for c = [0.4, 0.9]
    ...
    for M = [21, 31, 51]
        ...
        imshow(Y);
        m = M;
        caption = sprintf('Added noise to image, M= %d, b= %d ', m, c);
        title(caption, 'FontSize', 14);
        imageno = imageno + 1;
    end
end
...
for M = [21, 31, 51]
    ...
    figure(imageno);
    plot(W,abs(H));
    caption = sprintf('Frequency Response, M= %d', M);
    title(caption, 'FontSize', 14);
...
figure(imageno);
imshow(Y);
caption = sprintf('Applying First Differencer');
title(caption, 'FontSize', 14);
imageno = imageno + 1;
